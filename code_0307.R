...
da.genrt=function(N){
  ...
  K=21 #number of repetition
  
  # parameters for the f(t)
  betas <- cbind(-2,0.2)
  sigma.y <- 0.3 # measurement error standard deviation
  D=matrix(c(0.1,0.04,0.04,0.1),nrow=2) # random effects bi = (bi1,bi2)^T ~ N(0,D]) **diff from Lin et al.(0.4, 0.1, 0.1, 0.2)
  ...
  times <- c(replicate(400, c(0,seq(0.5,10,0.5))))
...
fpca.fit<- function(data){
  ...
  summary(result)     # try 함수 유무 차이 없음. 
  ...
  mwfdata.id=mwfdata[!duplicated(mwfdata$id),]     ### 여기가지 봄 .t=2일떄 시뮬레이션돌림 
  ...
  for(k in mwfdata.id[,1]){
    for(t in t0vec){
      if(sum(mwfdata$id==k & mwfdata$tij==t)){     ## mwfdata$id==k & mwfdata$tij==t 이 경우만 dataall fpca score 데이터를 mwfdata 데이터로 변경
    ...
    dataall[dataall$id==k,7]= mwfdata[which(mwfdata$id==k)[1] ,]$event      ## 다른 변수들은 조건없이 mwfdata 로 대체
  ...
  obj=list(da=da,ni.vec=ni.vec,Umat=Umat,Yvec=Yvec,tvec=tvec,visit.mat=visit.mat,dataall=dataall)
  return(obj)
}                          ## 1/30 여기까지 봄
...
fpca.analysis <- function(obj)
{
  ...
  for(i in 1:n0){
    ...
    wt.curr2=(da$delta[i]*(da$Y[i]>tseq)/invw.curr)
  ...
  return(list(coef=temp.est, gamma.fit = gamma.fit, come.cur = come.cur , wt.reg = wt.reg, wt.reg.2 = wt.reg.2, da=da , data=dataall))
...
### Yvec indicatior 이랑 Umat 999 를 계산할 때 0이 되도록 수정한 식 ### 


# objectF = function(beta){
#   beta = as.matrix(beta)
#   
#   result = (1/N)*Ind*t(U.rev*come) %*% (W*(pnorm((U %*% beta-logY)/sqrt(diag(U %*% H %*% t(U)))))-tau0)
#   result = as.vector(result)
# }
...
# Ind <- ifelse(exp(logY) > t, 1, 0)
# U.rev <- ifelse(U==999 , 0 , U)

#############################

betastart=c(fit.fpca$gamma.fit)
...
tryCatch({
  if (is.fit$convergence == 0){  ## 수렴했는지
    ...
    idw.beta1.est=as.vector(idw.splinef%*%idw.fit[(2*nbasis+1):(3*nbasis)])
    idw.beta2.est=as.vector(idw.splinef%*%idw.fit[(3*nbasis+1):(4*nbasis)])
    idw.beta3.est=as.vector(idw.splinef%*%idw.fit[(4*nbasis+1):(5*nbasis)])
...
temp.est
idw.temp.est
temp.est == idw.temp.est


exp(0.4*(-2+0.2*mean(t)))




#######################################################
# Point estimate: tryCatch 사용하지 말고 수렴하지 않는 경우 어떤 종류의 에러 메시지가 뜨는지 그리고 output 관찰해보면서 문제 있는 부분 수정할 것
#######################################################
...
for (i in sample_num) {  # 100번의 반복을 위한 for문
